# -*- coding: utf-8 -*-
"""compute_caudal.py —— 尾鳍摆动频率【一致性校验】
==========================================================================
逻辑已**不在本脚本实现**，统一由 postproc.py 的 caudal 配置提供
（`smooth_K=1`；2026-09-15 起 dedupe=True, ffill=False；同态摆动 swing + 间隔 ≥30 帧；
现行 21-1 人眼 AR=82.74%, r=+0.84；历史 2026-09-14 为 AR=79.55%, r=+0.86）。
本脚本作用：用同一逻辑重算 → 与权威合并表**逐格比对**，确认口径一致。
输入 : P.QUANT/4_尾鳍.xlsx
权威 : P.QUANT/尾鳍摆动频率.xlsx（由 combine_15_sheets.py 合并）
输出 : **不落盘**（2026-09-15 起）。此前输出 `P.QUANT/尾鳍摆动频率_重算.xlsx`，
...
REF_XLSX = os.path.join(RES_DIR, "尾鳍摆动频率.xlsx")
