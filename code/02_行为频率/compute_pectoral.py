# -*- coding: utf-8 -*-
"""compute_pectoral.py —— 胸鳍摆动频率【一致性校验】
==========================================================================
逻辑已**不在本脚本实现**，统一由 postproc.py 的 pectoral 配置提供
（`smooth_K=1`；2026-09-15 起 dedupe=True, ffill=False；周期法 3→4→3，中间态 ≥2 帧；
现行 21-1 人眼 AR=81.16%, r=+0.78；历史 2026-09-14 为 AR=73.47%, r=+0.47）。
本脚本作用：用同一逻辑重算 → 与权威合并表**逐格比对**，确认口径一致。
输入 : P.QUANT/3_胸鳍.xlsx
权威 : P.QUANT/胸鳍摆动频率.xlsx（由 combine_15_sheets.py 合并）
输出 : **不落盘**（2026-09-15 起）。此前输出 `P.QUANT/胸鳍摆动频率_重算.xlsx`，
...
REF_XLSX = os.path.join(RES_DIR, "胸鳍摆动频率.xlsx")
