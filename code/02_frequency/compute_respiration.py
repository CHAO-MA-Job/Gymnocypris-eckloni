# -*- coding: utf-8 -*-
"""Consistency check for the opercular rate: recompute it and compare with the merged table.

The counting rule comes from postproc.py (cycle 1 -> 2 -> 1, intermediate state of at least two
frames). The script recomputes the rate from the region table and compares it cell by cell with
the merged workbook, which confirms that both use the same definition.

Input   : <work root>/opercular.xlsx
Against : <work root>/opercular/respiration.xlsx
...
REF_XLSX = os.path.join(RES_DIR, "respiration.xlsx")
